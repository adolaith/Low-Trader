...
public class EntityProfileLoader extends SaveLoadMenu {
	...
	public void saveProfile(){
		String id = IdGenerator.EDITOR_ONLY;
		
		if(((CheckBox)editor.getCheckBoxes().findActor("npc")).isChecked()){
			id += IdGenerator.getEntityId(IdType.NPC);
		}else if(((CheckBox)editor.getCheckBoxes().findActor("item")).isChecked()){
			id += IdGenerator.getEntityId(IdType.ITEM);
		}else if(((CheckBox)editor.getCheckBoxes().findActor("wall")).isChecked()){
			id += IdGenerator.getEntityId(IdType.WALL);
		}else if(((CheckBox)editor.getCheckBoxes().findActor("ent")).isChecked()){
			id += IdGenerator.getEntityId(IdType.ENTITY);
		}
